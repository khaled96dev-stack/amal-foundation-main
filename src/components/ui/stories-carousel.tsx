...
export const Story = ({ className, ...props }: StoryProps) => (
  <CarouselItem className="basis-auto pl-2 md:pl-4">
    <div
      className={cn(
        'group relative overflow-hidden rounded-[24px] bg-muted/40 h-full w-full',
        'cursor-pointer transition-all duration-500 ease-out',
        'hover:scale-[1.02] shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.16)]',
...
export type StoryImageProps = Omit<ComponentProps<typeof Image>, 'fill' | 'alt'> & {
  alt: string;
  sizes?: string;
};

export const StoryImage = ({ className, alt, sizes = '100vw', ...props }: StoryImageProps) => (
  <Image
    alt={alt}
    fill
    sizes={sizes}
...
export const StoryAuthorImage = ({
  ...
  <Avatar className={cn('size-8 border border-white/40', className)} {...props}>
...
export const StoryAuthorName = ({
  ...
  <span className={cn('truncate font-medium text-sm drop-shadow-md', className)} {...props} />
);
...
export const StoryOverlay = ({
  ...
  return (
    <div
      className={cn(
        'absolute right-0 left-0 h-24 from-black/60 to-transparent',
        positionClasses,
        className
      )}
      {...props}
